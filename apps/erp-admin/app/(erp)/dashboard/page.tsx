      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader
            ...
            action={
              <Link href="/commandes" className="flex items-center gap-1 text-xs font-medium text-brand-700 hover:text-brand-600">
          ...
          <Table>
            ...
            <tbody>
              {kpis.recentOrders.map((o) => (
                <TR key={o.id}>
                  <TD className="font-medium text-brand-800">{o.number}</TD>
                  <TD>{o.shippingFullName}</TD>
                  <TD>{money(o.totalTTC)}</TD>
                  <TD>
              ...
              {kpis.recentOrders.length === 0 ? (
                <TR>
                  <TD colSpan={4} className="py-8 text-center text-slate-400">Aucune commande</TD>
                </TR>
              ) : null}
            </tbody>
          </Table>
        ...
        <Card>
          <CardHeader
            ...
            action={
              <Link href="/factures" className="flex items-center gap-1 text-xs font-medium text-brand-700 hover:text-brand-600">
                Tout voir <ArrowUpRight className="h-3.5 w-3.5" />
              </Link>
            }
          />
          <Table>
            ...
            <tbody>
              {kpis.recentInvoices.map((inv) => (
                <TR key={inv.id}>
                  <TD className="font-medium text-brand-800">{inv.number}</TD>
                  <TD>{formatDate(inv.issueDate)}</TD>
                  <TD>{money(inv.totalTTC)}</TD>
                  <TD>
              ...
              {kpis.recentInvoices.length === 0 ? (
                <TR>
                  <TD colSpan={4} className="py-8 text-center text-slate-400">Aucune facture</TD>
                </TR>
              ) : null}
            </tbody>
          </Table>
