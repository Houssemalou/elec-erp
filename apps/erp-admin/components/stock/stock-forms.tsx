...
function CardForm({
  ...
  return (
    <div className="rounded-xl border border-[#2A2A2A] bg-[#151515] shadow-card">
      <div className="flex items-center gap-1 border-b border-[#2A2A2A] px-4 pt-3">
        {tabs.map((t) => (
          <button
            key={t.id}
            type="button"
            ...
            className={`inline-flex items-center gap-2 rounded-t-lg px-4 py-2.5 text-sm font-medium ${
              tab === t.id ? 'border-b-2 border-accent-400 text-accent-400' : 'text-white/50 hover:text-white/70'
            }`}
          >
            {t.icon} {t.label}
          </button>
      ...
      {rows.length > 0 ? (
        <div className="overflow-x-auto rounded-lg border border-[#2A2A2A]">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#2A2A2A] bg-[#0B0B0B] text-left text-xs font-semibold uppercase tracking-wide text-white/50">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.productId} className="border-b border-[#2A2A2A]">
                  <td className="px-3 py-1.5 font-mono text-xs text-white/50">{r.sku}</td>
                  <td className="px-3 py-1.5 text-white">{r.name}</td>
                  <td className="px-3 py-1.5 text-right text-white/60">{r.theoretical.toLocaleString('fr-FR')}</td>
        ...
        <p className="text-sm text-white/40">Choisissez un dépôt pour saisir l&apos;inventaire.</p>
      )}
